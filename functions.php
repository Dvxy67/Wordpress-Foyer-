...
/**
 * ========================================
 * AJOUTER CE CODE À LA FIN DE functions.php
 * ========================================
 * 
 * Configuration des images de retour pour toutes les pages
 */

/**
 * Customizer pour les images de retour - À ajouter dans functions.php
 */
function pages_retour_images_customizer($wp_customize) {
    
    // ========================================
    // PAGE FOYER - Image Retour
    // ========================================
    ...
    $wp_customize->add_control(new WP_Customize_Image_Control($wp_customize, 'foyer_retour_image', array(
        ...
        'description' => 'Image de la flèche de retour (recommandé: 90x90px)',
    )));

    // ========================================
    // PAGE QUARTIERS - Image Retour
    // ========================================
    $wp_customize->add_setting('quartiers_retour_image', array(
        'default' => '',
        'sanitize_callback' => 'esc_url_raw',
    ));
    $wp_customize->add_control(new WP_Customize_Image_Control($wp_customize, 'quartiers_retour_image', array(
        'label' => 'Image Flèche Retour',
        'section' => 'quartiers_section',
        'settings' => 'quartiers_retour_image',
        'description' => 'Image de la flèche de retour (recommandé: 90x90px)',
    )));

    // ========================================
    // PAGE LOGEMENT SOUS-MENU - Image Retour
    // ========================================
    ...
    $wp_customize->add_control(new WP_Customize_Image_Control($wp_customize, 'pannes_retour_image', array(
        'label' => 'Image Flèche Retour (Globale)',
        'section' => 'pannes_global_section',
        'settings' => 'pannes_retour_image',
        'description' => 'Image de la flèche de retour pour toutes les pages pannes (recommandé: 90x90px)',
    )));
}
add_action('customize_register', 'pages_retour_images_customizer');

/**
 * Helper pour récupérer l'image de retour d'une page
 * Fallback vers une flèche Unicode si pas d'image
 */
function get_retour_image($page_type, $quartier = null) {
    $image = '';
    
    // Déterminer quelle option récupérer selon le type de page
    switch ($page_type) {
        case 'habitants':
            $image = get_theme_mod('habitants_retour_image', '');
            break;
        case 'foyer':
            $image = get_theme_mod('foyer_retour_image', '');
            break;
        case 'quartiers':
            $image = get_theme_mod('quartiers_retour_image', '');
            break;
        case 'logement':
            $image = get_theme_mod('logement_retour_image', '');
            break;
        case 'pannes':
            // Pour les pannes, il y a un fallback global
            if ($quartier) {
                $image = get_theme_mod("pannes_{$quartier}_retour_image", '');
            }
            if (empty($image)) {
                $image = get_theme_mod('pannes_retour_image', '');
            }
            break;
    }
...
/**
 * Affiche la flèche de retour (image ou fallback unicode)
 */
function display_retour_arrow($page_type, $quartier = null) {
    $image = get_retour_image($page_type, $quartier);
    
    if ($image) {
        echo '<img src="' . esc_url($image) . '" alt="Retour" class="arrow-image" aria-hidden="true">';
    } else {
